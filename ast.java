    ...
    public void typeCheck(Type returnType) {
        for (StmtNode s : myStmts) {
            if (s instanceof ReturnStmtNode) {
                ((ReturnStmtNode)s).typeCheck(returnType);
            } else {
                s.typeCheck();
            }
        }
...
class ExpListNode extends ASTnode {
    ...
    public List<Info> typeCheck() {
        List<Info> infoList = new LinkedList<>();
        for (ExpNode e : myExps) {
            Type expType = e.typeCheck();
            infoList.add(new Info(e.typeCheck(), e.lineNum(), e.charNum()));
        }
        return infoList;
    }
...
class Info {
    public Info(Type type, int lineNum, int charNum) {
        this.type = type;
...
class WriteStmtNode extends StmtNode {
    ...
    public void typeCheck() {
        ...
        if (t.isErrorType()) {
            ...
            ErrMsg.fatal(((CallExpNode)myExp).lineNum(), ((CallExpNode)myExp).charNum(), "Attempt to write void");
        }
    }
...
class AssignNode extends ExpNode {
    ...
    public Type typeCheck() {
        ...
        } else if (t1.equals(t2) && (t1.isBoolType() || t1.isIntType())) {
            return t1;
        } else {
...
class CallExpNode extends ExpNode {
    ...
    public Type typeCheck() {
        TSym s = myId.sym();
        if (s.getType().equals(new FnType())) {
            List<Type> fnList = ((FnSym) s).getParamTypes();
            List<Info> formalList = myExpList.typeCheck();
            if (fnList.size() != formalList.size()) {
                ErrMsg.fatal(myId.lineNum(), myId.charNum(), "Function call with wrong number of args");
            } else {
                for (int i = 0; i < fnList.size(); ++i) {
                    Info formalInfo = formalList.get(i);
                    if (formalInfo.type.isErrorType()) {
                    } else if (!fnList.get(i).equals(formalInfo.type)) {
                        ErrMsg.fatal(formalInfo.lineNum, formalInfo.charNum, "Type of actual does not match type of formal");
                    }
                }
            }

            Type fnReturnType = ((FnSym) s).getReturnType();
            return fnReturnType;
        } else {
...
abstract class BinaryExpNode extends ExpNode {
    ...
    public Type typeCheck() {
        ...
        if (this instanceof PlusNode || this instanceof MinusNode || this instanceof TimesNode || this instanceof DivideNode) {
        ...
        } else if (this instanceof AndNode || this instanceof OrNode) {
        ...
        } else if (this instanceof EqualsNode || this instanceof NotEqualsNode) {
            if (t1.isErrorType() || t2.isErrorType()) {
            } else if (t1.isFnType() && t2.isFnType()) {
        ...
        } else if (this instanceof LessNode || this instanceof GreaterNode || this instanceof LessEqNode || this instanceof GreaterEqNode) {
            if (t1.isIntType() && t2.isIntType()) {
                return new BoolType();
            }
            if (!t1.isErrorType() && !t1.isIntType()) {
